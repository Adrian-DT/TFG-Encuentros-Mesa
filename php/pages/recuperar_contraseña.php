...
<head>
    ...
    <title>Recuperar contraseña | RDM</title>
...
<body>
    <header class="text-center">
        ...
        <div class="container-fluid mt-3 pt-5 pb-2 contHeader">
            ...
            <p class="text-light">Recupera tu contraseña.</p>
        </div>
    </header>
    <main class="mt-2 py-2">
        <h1 class="text-center py-2">Recuperación de contraseña</h1>
        <div class="container d-flex text-center justify-content-center">
            <?php
            if (!isset($_GET["email"])) {
                echo "<form action='../functions/controlador_formularios.php' method='POST'>
                                <div class='form-group'>
                                    <label for='email'>Email:</label>
                                    <input type='text' class='form-control form-control-sm' id='email' name='email' placeholder='Escribe tu email'>
                                </div>
                                <button type='submit' class='btn btn-success my-4' name='recuperar_contraseña'>Enviar correo de recuperación</button>
                            </form>";
            } else if ($_GET["email"] == "no_existe") {
                echo "<div class='d-flex flex-column'>
                            <form action='../functions/controlador_formularios.php' method='POST'>
                                <div class='form-group'>
                                    <label for='email'>Email:</label>
                                    <input type='text' class='form-control form-control-sm' id='email' name='email' placeholder='Escribe tu email' value='" . htmlspecialchars($_GET['email_user']) . "'>
                                </div>
                                <button type='submit' class='btn btn-success my-4' name='recuperar_contraseña'>Enviar correo de recuperación</button>
                            </form>
                            <p style='max-width: 220px;'>El email proporcionado no existe,<br> revise el email introducido e inténtelo de nuevo.</p>
                        </div>
                            ";
            } else {
                echo "<div>
                            <h5>Hemos enviado un correo de recuperación de contraseña.</h5><br><br>
                            <p>Consulte su bandeja de correo para reestablecer una nueva contraseña para su cuenta.</p>
                        </div>
                    ";
            }
            ?>
